...
public class Helper {
	...
	public static Datacenter createDatacenter(String name,
			...
			List<? extends PowerHost> hostList, VmAllocationPolicy vmAllocationPolicy)
			throws Exception {
	...
	public static List<IoVm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			...
			if (Constants.REAL_IO) {
				try {
					File inputFolder = new File(Constants.INPUT_IO_WORKLOAD_DIR);
				...
				ioModel = (IoWorkloadModelInMemory)Constants.ioWorkload1;
			}
