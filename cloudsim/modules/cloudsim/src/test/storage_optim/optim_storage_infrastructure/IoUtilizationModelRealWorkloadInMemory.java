 ...
 */
public class IoUtilizationModelRealWorkloadInMemory implements UtilizationModel {
	 ...
	 */
	public IoUtilizationModelRealWorkloadInMemory(String inputPath, double schedulingInterval)
		...
		int n = data.length;
		for (int i = 0; i < n - 1; i++) {
			data[i] = Double.parseDouble(input.readLine()) / 100.0;
		}
		data[n - 1] = data[n - 2];
		input.close();
	}
	...
	public double getUtilization(double time) {
		if (time % getSchedulingInterval() == 0) {
			//Log.printLine("HAMZA: utilization "+data[(int) time / (int) getSchedulingInterval()]);
