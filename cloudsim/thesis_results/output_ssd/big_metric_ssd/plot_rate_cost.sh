...
echo "
        ...
        set xlabel \"Seuil Max d'utilisation\"
        set ylabel \"Coût total (\$)\"
        set yrange [0:*]
        set xrange [-1:6]
		set format x \"%2.0f%%\" 
		set grid
		set key inside top right horizontal box
		set key font 'Helvetica Bold,14'
		set key width -1.5
		set key samplen 2
		set key spacing 2
		set grid
        plot \"./$FILE1\" using 4:xticlabels(3) with lp ps 1 lw 2 ti \"Glouton\" ,\
        \"./$FILE1\" using 6:xticlabels(3) with lp ps 1 lw 2 ti \"HPSD\" ,\
        \"./$FILE1\" using 8:xticlabels(3) with lp ps 1 lw 2 ti \"HPPD\" ,\
        \"./$FILE1\" using 10:xticlabels(3) with lp ps 1 lw 2 ti \"Sans stockage\" " | gnuplot
