...
import org.cloudbus.cloudsim.examples.power.RunnerAbstract_original;
 ...
 * @since Jan 5, 2012
 */
public class PlanetLabRunner extends RunnerAbstract_original {
	...
	protected void init(String inputFolder) {
		try {
			...
			broker = Helper_original.createBroker();
			int brokerId = broker.getId();

			cloudletList = PlanetLabHelper.createCloudletListPlanetLab(brokerId, inputFolder);
			vmList = Helper_original.createVmList(brokerId, cloudletList.size());
			hostList = Helper_original.createHostList(PlanetLabConstants.NUMBER_OF_HOSTS);
		} catch (Exception e) {
			e.printStackTrace();
