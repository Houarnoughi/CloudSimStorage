...
import org.cloudbus.cloudsim.Storage_original;
...
public class Helper_original {
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			int vmType = i / (int) Math.ceil((double) vmsNumber / Constants_original.VM_TYPES);
			vms.add(new PowerVm(
					i,
					brokerId,
					Constants_original.VM_MIPS[vmType],
					Constants_original.VM_PES[vmType],
					Constants_original.VM_RAM[vmType],
					Constants_original.VM_BW,
					Constants_original.VM_SIZE,
					1,
					"Xen",
					new CloudletSchedulerDynamicWorkload(Constants_original.VM_MIPS[vmType], Constants_original.VM_PES[vmType]),
					Constants_original.SCHEDULING_INTERVAL));
		}
		return vms;
	}
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			int hostType = i % Constants_original.HOST_TYPES;

			List<Pe> peList = new ArrayList<Pe>();
			for (int j = 0; j < Constants_original.HOST_PES[hostType]; j++) {
				peList.add(new Pe(j, new PeProvisionerSimple(Constants_original.HOST_MIPS[hostType])));
			}

			hostList.add(new PowerHostUtilizationHistory(
					i,
					new RamProvisionerSimple(Constants_original.HOST_RAM[hostType]),
					new BwProvisionerSimple(Constants_original.HOST_BW),
					Constants_original.HOST_STORAGE,
					peList,
					new VmSchedulerTimeSharedOverSubscription(peList),
					Constants_original.HOST_POWER[hostType]));
		}
		return hostList;
	}
	...
	public static Datacenter createDatacenter(
		...
		try {
			datacenter = datacenterClass.getConstructor(
					...
					characteristics,
					vmAllocationPolicy,
					new LinkedList<Storage_original>(),
					Constants_original.SCHEDULING_INTERVAL);
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
